  ...
  function OtherRequest(props) {
    ...
    const doSubmit = async (values, { resetForm }) => {
      ...
      if (profile) {
        setSendValues({
          ...
          coo_code: 0,
          coo_status_type: "O",//O >> Other Request
          income_code: values.serviceType,
          remarks: values.serviceType,
    ...
    useEffect(() => {
      if (APIServices.uploadRakFileDone?.length) {
        ...
        dispatch(sendOtherRequest({ data: { ...sendValues } }));
      }
    ...
    useEffect(() => {
      const result = APIServices.otherRequestDone;
      if (result.rowcode) {
    ...
    useEffect(async () => {
      ...
      if (Object.keys(APIServices.otherRequestDone).length) {
        dispatch(sendOtherRequestDone({ data: [] }));
