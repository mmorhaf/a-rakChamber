import {
  ...
  function OtherRequest(props) {
    ...
    const initialValues = editingInitialValues("editOther", requestData);
    ...
    const doSubmit = async (values, { resetForm }) => {
      ...
      if (profile) {
        ...
        if (cooAttachment && cooAttachment?.length > 0) {
          ...
          dispatch(sendOtherRequestUpdate({ data: { ...readyValues } }));
        }
        // resetForm({});
        setNote("");
      } else {
    ...
    useEffect(() => {
      ...
      dispatch(getOtherRequestDetails({ data: { code } }));
    }, []);
    ...
    useEffect(() => {
      if (APIServices.uploadRakFileDone?.length) {
        ...
        dispatch(sendOtherRequestUpdate({ data: { ...sendValues } }));
      }
    ...
    useEffect(() => {
      const result = APIServices.otherRequestUpdated;
      if (result.reqCode) {
    ...
    }, [APIServices.otherRequestUpdated]);
    ...
    useEffect(async () => {
      ...
      if (Object.keys(APIServices.otherRequestUpdated).length) {
        dispatch(sendOtherRequestUpdateDone({ data: {} }));
        dispatch(uploadRakFileDone({ data: [] }));
      }
    }, [APIServices.cooAttachments]);
  
    useEffect(() => {
      const result = APIServices.otherRequestDetails;
      if (result?.code) {
        ...
        let other_request_code = code;
        let coo_code = result.coo_code;
        // let trx_code = 35223;
        // let other_request_code = 762;
        // let coo_code = 47284;
        if (status == "issued")
          dispatch(
            getCooStamps({
              data: { trx_code, other_request_code, coo_code },
            })
          );
      }
    }, [APIServices.otherRequestDetails]);
    useEffect(() => {
        ...
        <Box>
          ...
          >
            ...
            <Typography className={classes.serviceTitle}>
              {t("SERVICESPAGES.OTHERREQ.TITLE")}
            </Typography>
                ...
                <Grid container spacing={1}>
                  ...
                  <Grid item md={6} xs={12} className={classes.inpuContainer}>
                    <InputLabel htmlFor="serviceType" className={classes.label}>
                      {t("SERVICESPAGES.OTHERREQ.TYPE")}{" "}
                       
                      <span style={{ color: "red" }}>*</span>
                      <span style={{ color: "red" }}>cooTypeCode = {cooTypeCode}</span>
                    </InputLabel>
                    <Field
                      component={Select}
                    ...
                    >
                      ...
                      >
                        {t("SERVICESPAGES.OTHERREQ.OTHERORIGIN(FORMAPAPERS)")}
                      </MenuItem>
                      <MenuItem
                        key={2}
                        value={60}
                        ...
                        className={classes.menuItem}
                      >   
                        {t("SERVICESPAGES.OTHERREQ.CORRECTIONSTAMP")}
                      </MenuItem>
                      ...
                      <MenuItem
                        ...
                        className={classes.menuItem}
                      >   
                        {t("SERVICESPAGES.OTHERREQ.ORIGINALSEAL")}
                      </MenuItem>
                      ...
                      <MenuItem
                        ...
                        className={classes.menuItem}
                      >   
                        {t("SERVICESPAGES.OTHERREQ.LIVESEAL")}
                      </MenuItem>
                  ...
                  <Grid item md={6} xs={12} className={classes.inpuContainer}>
                    <InputLabel htmlFor="count" className={classes.label}>
                      {t("SERVICESPAGES.OTHERREQ.COUNT")}
                      <span style={{ color: "red" }}>*</span>
                    </InputLabel>
                    <Field
                      component={TextField}
                  ...
                  <Grid item md={6} xs={12} className={classes.inpuContainer}>
                    ...
                    <Field
                      ...
                      value={
                        type == "view"
                          ? requestData?.fees
                          :values.serviceType == "59"
                          ? values.count * 10
                          : values.serviceType == "60"
                          ? values.count * 50
                          : values.serviceType == "61"
                          ? values.count * 50
                          : values.serviceType == "62"
                          ? values.count * 50
                          : ""
                  ...
                  >
                    <InputLabel htmlFor="notes" className={classes.label}>
                      {t("SERVICESPAGES.OTHERREQ.NOTES")}
                    </InputLabel>
                    <textarea
                      rows={5}
                      ...
                      <>
                        ...
                        <Box className={classes.divider}>
                          <Typography name="fills">
                            {" "}
                            {t("SERVICESPAGES.OTHERREQ.ATTACHMENT")}
                          </Typography>
                        </Box>
  ...
  export default memo(OtherRequest);
